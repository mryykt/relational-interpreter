...
(define (run-test)
  (test "cons"
    (run 2 (q) (minio '(cons 1 (list ())) '() q))
    '((list (1))))
  (test "function"
    (run 1 (q) (minio '(app (lambda x x) 1) '() q))
    '(1))
  (test "case-list"
    (run 1 (q) (minio '(case (list (1 2 3)) (list ()) (lambda ca (lambda cd cd))) '() q))
    '((list (2 3))))
  (test "case-list-null"
    (run 1 (q) (minio '(case (list ()) 1 (lambda ca (lambda cd cd))) '() q))
    '(1))
  (test "let"
    (run 1 (q) (minio '(let x 1 x) '() q))
    '(1))
  (test "integration"
    (run 1 (q) (minio '(let
          f
          (lambda xs (case xs (list ()) (lambda ca (lambda cd (cons (cons ca (list ())) (app f cd))))))
          (app f (list (1 2 3))))
        '() q))
